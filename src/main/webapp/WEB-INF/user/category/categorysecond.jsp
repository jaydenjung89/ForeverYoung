...
<div id="Container">
	...
	<div id="Contents">
		...
		<c:forEach var="categorySecond" items="${categorySecond}">
			<c:if test="${secondSerial == categorySecond.category_second_serial}">
			<li class="">
				<a href="categoryItemPage2.do?category_first_serial=${categoryFirst.category_first_serial}&category_second_serial=${categorySecond.category_second_serial}" class="" > 전체</a>
			</li>
			<c:forEach var="categoryThird" items="${categoryThird}">
			
			<c:if test="${thirdSerial == categoryThird.category_third_serial }">
			<li class="on">
				<a href="categoryItemPage2.do?category_first_serial=${categoryFirst.category_first_serial}&category_second_serial=${categorySecond.category_second_serial}&category_third_serial=${categoryThird.category_third_serial}" >${categoryThird.category_third_name}</a>
			</li>
			</c:if>
			...
			<c:if test="${thirdSerial != categoryThird.category_third_serial }">
			<li class="">
				<a href="categoryItemPage2.do?category_first_serial=${categoryFirst.category_first_serial}&category_second_serial=${categorySecond.category_second_serial}&category_third_serial=${categoryThird.category_third_serial}" class="" >${categoryThird.category_third_name}</a>
			</li>
			</c:if>
		...
		<c:forEach var="item" items="${categoryProductList}" varStatus="num">
			...
			<li criteo-goods="${item.category_goods_serial}" class="flag" data-index="${num.index}">
				<input type="hidden" class="" value="${item.category_goods_serial}"/>
				<div class="prd_info ">
					<!-- 상품 바로가기 링크 -->
					<a href="itemPage.do?category_first_serial=${item.category_first_serial}&category_second_serial=${item.category_second_serial}&category_third_serial=${item.category_third_serial}&category_goods_serial=${item.category_goods_serial}" name="" class="prd_thumb goodsList" data-ref-goodsno=""  onclick="" >
						<!-- 베스트 상품 표시 -->
						<span class="thumb_flag best">${item.category_goods_info }</span>
						<!-- 상품이미지 -->
						<img src="${item.category_goods_thum_img}"  onerror="common.errorImg(this);">
					...
					<!-- 상품설명 -->
					<div class="prd_name">					
						<a href="itemPage.do?category_first_serial=${item.category_first_serial}&category_second_serial=${item.category_second_serial}&category_third_serial=${item.category_third_serial}&category_goods_serial=${item.category_goods_serial}" class="goodsList" onclick="">
							<!-- 브랜드 이름 -->
							<span class="tx_brand">${item.category_goods_brand}</span>
							<!-- 상품이름 -->
					...
					<p class="prd_price">
						...
						<span class="tx_org"><span class="tx_num">${item.goods_detail_price}</span>원 </span>
						<!-- 상품현재가 -->
						<span class="tx_cur"><span class="tx_num">${lastPrice}</span>원 </span>
					</p>
			...
			<!-- 상품객체 끝 -->
			<c:if test="${num.count % '4'=='0' }"></ul></c:if>			
			</c:forEach>
